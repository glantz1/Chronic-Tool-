...
class Intervention(db.Model):
  ...
  student_record_id = db.Column(
      db.Integer, db.ForeignKey('student_record.id'), nullable=False
  )
  action_type = db.Column(
      db.String(100), nullable=False, default='General Support'
  )
...
def index():
  ...
  available_grades = [
      g[0]
      for g in db.session.query(StudentRecord.grade).distinct().all()
      if g[0]
  ]
...
def log_intervention(student_id):
  ...
  if user.role != 'Admin' and student.school_id != user.school_id:
    flash('Unauthorized action for this student record.', 'error')
    return redirect(url_for('index'))

  notes = request.form.get('notes', '').strip()
  if notes:
    intervention = Intervention(
        student_record_id=student.id, notes=notes, logged_by=user.username
    )
...
def upload_csv():
  ...
  if user.role == 'Admin':
    school_id = request.form.get('school_id')
    if not school_id:
      flash('Admin must select a school target for CSV upload.', 'error')
      return redirect(url_for('index'))
  ...
  if not target_school_id:
    flash('Your account is not linked to any specific school.', 'error')
    return redirect(url_for('index'))
  ...
  for row in reader:
    ...
    absences = float(row.get('absences', row.get('Total Absences', 0.0)))
    unexcused = int(
        row.get('unexcused_absences', row.get('Unexcused Absences', 0))
    )
    tardies = int(row.get('tardies', row.get('Tardies', 0)))
    total_days = float(row.get('total_days', row.get('Total Days', 180.0)))
    grade = row.get('grade', row.get('Grade', 'N/A'))

    present_fte = (
        ((total_days - absences) / total_days * 100) if total_days > 0 else 0.0
    )

    existing = StudentRecord.query.filter_by(
        student_id=student_id, school_id=target_school_id
    ).first()
    if existing:
      existing.name = name
...
# ------------------------------------------------------------------------------
# App Initialization & Default Admin Setup
# ------------------------------------------------------------------------------
